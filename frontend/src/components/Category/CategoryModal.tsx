...
export const CategoryModal: React.FC<CategoryModalProps> = ({ isOpen, onClose, editCategory }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editCategory) {
        await updateCategory(editCategory.id, name.trim());
  ...
  return (
    <Modal
      ...
      description={editCategory ? 'Update your category details' : 'Add a new category classification'}
      size="sm"
    >
        ...
        <div>
          ...
          <input
            ...
            className={`w-full px-3.5 py-2 rounded-lg border text-sm outline-none transition-colors ${
              error ? 'border-red-300 bg-red-50/50 text-red-950 placeholder-red-300 focus:border-red-400' : 'border-gray-200 bg-gray-50/50 focus:border-indigo-500'
            }`}
            autoFocus
          />
          {error && (
